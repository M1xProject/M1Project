...
void ChatWindow::connecte()
{
    ...
    ui->tab->setTabToolTip(ui->tab->count()-1,"sinisalo.freenode.net");
    // current tab is now the last, therefore remove all but the last
    for (int i = ui->tab->count(); i > 1; --i) {
       ui->tab->removeTab(0);
    }

    serveurs.insert("sinisalo.freenode.net",serveur);

    serveur->pseudo=ui->editPseudo->text();
    serveur->serveur="sinisalo.freenode.net";
    serveur->port=6667;
    ...
    serveur->connectToHost("sinisalo.freenode.net",6667);
