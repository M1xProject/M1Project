...
unsigned int nTargetSpacing = 64; // 64 minute
...
const string strMessageMagic = "M1 Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // M1: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // M1: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight <= 2000) {
        nSubsidy = 52 * COIN;
		int factor = (pindexBest->nHeight / 200) + 1;
		nSubsidy /= factor;
    }else{
        return nFees;
    }
...
uint64_t GetInterestRate(const CBlockIndex* pindexLast, bool wholeCents)
{
    ...
    if (pindexLast && pindexLast->nHeight > 5000)
...
// M1: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // M1: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // M1: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // M1: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // M1: track money supply and mint amount info
...
// M1: total coin age spent in transaction, in the unit of coin-days.
...
// M1: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // M1: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // M1: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // M1: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // M1: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight > nLastPowBlock)
        return DoS(100, error("AcceptBlock() : reject proof-of-work at height %d <= %d", nHeight, nLastPowBlock));
    ...
    // M1: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // M1: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // M1: getblocks may not obtain the ancestor block rejected
    ...
    // M1: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "M1", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Welcome to M1! Backed by the newest technology & most updated code base!";
        CTransaction txNew;
        txNew.nTime = 1431329430;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 6;
        block.nTime    = 1431329430;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = fTestNet ? 38511 : 38511;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x765081b404d4439347d5ba0dc422d3b5a48a13aa0f8affb572741c92b22bed0c"));
        ...
        // M1: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // M1: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // M1: tell downloading node about the latest block if it's
